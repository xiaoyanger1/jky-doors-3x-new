...
namespace Young.Core.Data
{
    ...
    public class Serializer
    {
        ...
        public static string ConvertToXmlWithNoNameSpace(object objectToConvert)
        {
            string xml = null;
            ...
            if (objectToConvert != null)
            {
                ...
                XmlSerializer ser = new XmlSerializer(t, string.Empty);
                XmlSerializerNamespaces ns = new XmlSerializerNamespaces();
        ...
        public static string XmlDocToPureXmlSegment(string xmlDoc)
        {
            var rootElement = XDocument.Parse(xmlDoc).Root;
            rootElement.RemoveAttributes();
            var xmlSegment = rootElement.ToString(SaveOptions.DisableFormatting);

            return xmlSegment;
        }


        public static string ConvertToString(object objectToConvert, string encoding)
        {
            string xml = null;
            ...
            if (objectToConvert != null)
            {
                ...
                using (StringWriter writer = new StringWriter(CultureInfo.InvariantCulture))
                {
                    XmlWriterSettings s = new XmlWriterSettings() { Encoding = System.Text.Encoding.GetEncoding(encoding) };
                    XmlWriter xmlw = XmlWriter.Create(writer, s);
                    ser.Serialize(xmlw, objectToConvert);
                    xml = writer.ToString();
                    writer.Close();
                }
            }
